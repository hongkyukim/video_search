...
class Video < ActiveRecord::Base
  ...
  def self.get_OneVideoSearch(f)
	...
	case f.options
	when 'most_viewed'
            res = yt_session.videos_by(:most_viewed, :max_results => 25, :page => 1 )
	when 'most_popular'
            res = yt_session.videos_by(:most_popular, :max_results => 25, :page => 1)
	when 'top_rated'
            res = yt_session.videos_by(:top_rated, :max_results => 25, :page => 1)
	when 'recently_featured'
            res = yt_session.videos_by(:recently_featured, :max_results => 25, :page => 1)
	when 'top_favorites'
            res = yt_session.videos_by(:top_favorites, :max_results => 25, :page => 1)
	else
            begin
	         ###logger.debug "Alert: This option #{f.options} is not supported."
                 res = yt_session.videos_by(video_yt_options(f))
                 ###res = yt_session.videos_by(:query => "lion", :time => :month)
