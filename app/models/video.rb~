...
###@ytVideo = YouTubeVideo.new
  ...
  def self.add_video(v, channel_id)
    ...
    if mvideo.empty?
       ...
       @newvideo = Video.create(:title => v.title, :description => v.description, :yt_video_id => split_video_id[3],  :provider => prov, :thumbnail_url => v.thumbnails[0].url, :duration => v.duration, :categories => categories, :keywords => v.keywords)
